...
- **Secrets hygiene**: `.env` is gitignored and has never been committed; source only references `process.env.*`. `MCP_TOKEN` lives in `~/.config/incighder/mcp.env` (0600) and is referenced, never inlined, by MCP client configs. The data-api requires `X-Data-Api-Secret` on everything but `/health`, and **fails closed** (503) if `DATA_API_SECRET` is unset rather than serving open. The three routes that spawn the Claude CLI (`/agent_turn`, `/web_search`, `/extract_doc`) share a global rate limit, a concurrency cap, and a per-turn `--max-budget-usd` ceiling; `/agent_turn` additionally runs the CLI with no tools at all (`--tools '' --allowed-tools '' --restricted --permission-prompts none`). See the `CLAUDE_*` knobs in `.env.example`.
