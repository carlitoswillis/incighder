...
@app.route('/insert_artist_manual', methods=['POST'])
def insert_artist_manual():
    data = request.json
    ...
    try:
        ...
        print(f"Subprocess error (insert_artist_manual): STDOUT: {e.stdout}, STDERR: {e.stderr}", file=sys.stderr)
        return jsonify({'error': e.stderr.strip() if e.stderr else 'Unknown subprocess error'}), 500
    except json.JSONDecodeError as json_e:
        print(f"JSON decode error (insert_artist_manual): {json_e}. Raw stdout: {process.stdout}, Raw stderr: {process.stderr}", file=sys.stderr)
        return jsonify({'error': 'Invalid JSON response from script'}), 500
    except Exception as e:
        print(f"Unexpected error (insert_artist_manual): {e}", file=sys.stderr)
        return jsonify({'error': str(e)}), 500
...
def create_artist():
    ...
    try:
        ...
        print(f"Subprocess error (create_artist): STDOUT: {e.stdout}, STDERR: {e.stderr}", file=sys.stderr)
        return jsonify({'error': e.stderr.strip() if e.stderr else 'Unknown subprocess error'}), 500
    except json.JSONDecodeError as json_e:
        print(f"JSON decode error (create_artist): {json_e}. Raw stdout: {process.stdout}, Raw stderr: {process.stderr}", file=sys.stderr)
        return jsonify({'error': 'Invalid JSON response from script'}), 500
    except Exception as e:
        print(f"Unexpected error (create_artist): {e}", file=sys.stderr)
        return jsonify({'error': str(e)}), 500
